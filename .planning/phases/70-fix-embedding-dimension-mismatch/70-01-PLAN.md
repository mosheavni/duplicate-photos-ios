...
files_modified:
  ...
  - DuplicatePhotos/Services/DuplicateDetector.swift
...
must_haves:
  ...
  key_links:
    ...
    - from: "DuplicateDetector.scanForDuplicates"
      to: "CacheService.clearCache"
      via: "Explicit check for cache version mismatch before using cached embeddings"
      pattern: "clearCacheIfNeeded|cacheVersion"
---
...
<task type="auto">
  ...
  <files>DuplicatePhotos/Services/CacheService.swift, DuplicatePhotos/Services/DuplicateDetector.swift</files>
  <action>
Update CacheService to handle the dimension migration:

1. Add a cache version constant at the top of the actor:
   ```swift
...
3. In `loadCache()`, check version and auto-clear if mismatched:
   - After decoding, check if any entry has wrong version (or missing version field due to old format)
   - If version mismatch found, call `clearCache()` and log "Cleared cache due to version mismatch (old: X, current: Y)"

4. In `saveEmbedding()`, ensure new entries include current version

5. Add a `getCurrentCacheVersion() -> Int` method that returns the current cache version

6. Add a `getExpectedEmbeddingDimension() -> Int` method that returns 768 - this allows other services to validate embeddings

**CRITICAL WIRING: Update DuplicateDetector.scanForDuplicates():**

Because CacheService.init() loads cache asynchronously in a detached Task, the cache may not be validated by the time scanForDuplicates() runs. Add explicit cache validation at scan start:

In DuplicateDetector.swift, at the beginning of `scanForDuplicates()` (after the "Starting duplicate scan" log):
```swift
...
func ensureCacheValid() async {
    ...
    if hasInvalidEntries || cache.values.first?.version == nil {
...
Do NOT change the cache file location or encoding format.
  ...
  <verify>
1. Build succeeds: `xcodebuild -project DuplicatePhotos.xcodeproj -scheme DuplicatePhotos -destination 'platform=iOS Simulator,name=iPhone 16 Pro' build 2>&1 | tail -20`
2. Verify cache version field exists: `grep -n "cacheVersion" DuplicatePhotos/Services/CacheService.swift`
3. Verify version in CachedEmbedding struct: `grep -n "version: Int" DuplicatePhotos/Services/CacheService.swift`
4. Verify cache validation wiring: `grep -n "ensureCacheValid" DuplicatePhotos/Services/DuplicateDetector.swift`
  </verify>
  <done>
CacheService has version tracking (version=2), auto-clears old cache on dimension mismatch, and provides ensureCacheValid() method. DuplicateDetector.scanForDuplicates() explicitly calls ensureCacheValid() before using cached embeddings.
  </done>
</task>
...
<task type="checkpoint:human-verify" gate="blocking">
  <what-built>
Fixed embedding dimension mismatch (768 vs 512), added cache versioning with auto-clear, explicit cache validation wiring in DuplicateDetector, and updated similarity threshold to 0.92.
  </what-built>
  <how-to-verify>
...
6. Check Xcode console for diagnostic output:
   ...
   - If old cache existed, should see: "Clearing cache due to version mismatch"
...
- [ ] CacheService has cacheVersion constant set to 2
- [ ] CachedEmbedding struct includes version field
- [ ] CacheService.ensureCacheValid() method exists
- [ ] DuplicateDetector.scanForDuplicates() calls ensureCacheValid() at start
